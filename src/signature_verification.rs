...
pub(crate) async fn check_signature(signature_parts: SignatureParts, message: &[u8]) -> Result<()> {
    ...
    if signature_parts.message_hash_buffer.eq(context.finish().as_ref()) {
        let signature = Signature::from_asn1(signature_parts.signature_buffer.as_slice()).unwrap();

        let (_, cert) = parse_x509_certificate(signature_parts.public_keys_buffer[0].as_slice()).unwrap();

        let public_key = VerifyingKey::from_sec1_bytes(cert.tbs_certificate.subject_pki.subject_public_key.data).unwrap();
