...
pub fn extract_pdf_data(document: Vec<u8>) -> Result<(Vec<u8>, Vec<u8>, DateTime<FixedOffset>)> {
    ...
    let signature_element_start_position = document
        ...
        .context("Signature couldn't be found")?
        + signature_element_start_separator.len();

    let signature_date_start_separator = b"/M (D:";
    let signature_date_start_position = document
        .split_at(signature_element_start_position)
        .1
        .windows(signature_date_start_separator.len())
        .position(|window| window == signature_date_start_separator)
        .context("Signature date couldn't be extracted")?
        + signature_element_start_position
        + signature_date_start_separator.len();

    let signature_date_end_separator = b")";
    let signature_date_end_position = document
        .split_at(signature_date_start_position)
        .1
        .windows(signature_date_end_separator.len())
        .position(|window| window == signature_date_end_separator)
        .context("Signature date couldn't be extracted")?
        + signature_date_start_position;

    let signature_start_separator = b"/Contents <";
    let signature_start_position = document //.split_at(signature_date_end_position).1
        .windows(signature_start_separator.len())
        .position(|window| window == signature_start_separator)
        .context("Signature couldn't be extracted")?
        + signature_start_separator.len();

    let signature_end_separator = b">";
    let signature_end_position = document
        .split_at(signature_start_position)
        .1
        .windows(signature_end_separator.len())
        .position(|window| window == signature_end_separator)
        .context("Signature couldn't be extracted")?
        + signature_start_position;

    let byte_range_start_separator = b"/ByteRange [";
    let byte_range_start = document
        .split_at(signature_end_position)
        .1
        .windows(byte_range_start_separator.len())
        .position(|window| window == byte_range_start_separator)
        .context("Signature Message couldn't be extracted")?
        + signature_end_position
        + byte_range_start_separator.len();

    let byte_range_end_separator = b"]";
    let byte_range_end = document
        .split_at(byte_range_start)
        .1
    ...
    let signature_bytes = document.as_slice()[signature_start_position + 38..signature_end_position]
        .to_vec()
        .clone();

    let date = String::from_utf8_lossy(
        &document.as_slice()[signature_date_start_position..signature_date_end_position],
    )
        .to_string();
    ...
    let signature = hex::decode(String::from_utf8_lossy(signature_bytes.as_slice()).as_bytes())
        .context("Signature couldn't be extracted")?;
...
fn parse_byte_range(byte_range_descriptor: &[u8]) -> Vec<(usize, usize)> {
    byte_range_descriptor
        ...
        .fold(
            Vec::new(),
            |mut byte_slices: Vec<(Option<usize>, Option<usize>)>, limit| {
